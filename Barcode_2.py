...
def median_filter(image, kernel_size):
    ...
    for h in range(half_kernel_size, H - half_kernel_size):
        for w in range(half_kernel_size, W - half_kernel_size):
                sliced_img = image[h-half_kernel_size:h+half_kernel_size + 1, w-half_kernel_size:w+half_kernel_size + 1]
                filtered_img[h][w] = np.median(sliced_img * kernel)
    return filtered_img.astype(np.uint8)
...
if __name__ == "__main__":
    ...
    # 2. Median filtering 두 번 적용 (salt & pepper noise 제거)
    median_img = gray
    for i in range(2):
        median_img = median_filter(median_img, 5)

    # 3. Gaussian filtering (Smoothing)
    gaussian_img = gaussian_filtering(median_img)

    # 4. 수직 방향 edge detection
    vertical = cv2.filter2D(gaussian_img, -1, vertical_kernel)

    # 5. global thresholding으로 이진화
